  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      <div className="relative bg-gradient-to-br from-blue-600 to-purple-700 pt-6 pb-24 px-6 overflow-hidden">
        <img src={fondoUSM} alt="" className="absolute inset-0 w-full h-full object-cover opacity-20 pointer-events-none" />
        ...
        <div className="grid grid-cols-2 gap-3 mt-6 relative">
          <button onClick={() => onNavigate("escanear")} className="bg-white text-blue-700 rounded-xl p-5 shadow-lg hover:bg-gray-50 transition-colors">
            <HiCamera className="text-4xl mb-2 mx-auto" />
            <div className="text-sm font-bold">Escanear QR</div>
          </button>
          <button onClick={() => onNavigate("horarios")} className="bg-white text-purple-700 rounded-xl p-5 shadow-lg hover:bg-gray-50 transition-colors">
      ...
      <div className="max-w-sm mx-auto px-6 -mt-16 relative z-20">
        <Card className="mb-4 shadow-lg">
          <div className="flex items-center justify-between mb-3">
            ...
            {reservas.length > 0 && (
              <button onClick={() => onNavigate("reservas")} className="text-blue-600 text-sm font-medium hover:text-blue-700">
                Ver todas
              </button>
            )}
          </div>
          ...
          {reservas.length === 0 ? (
            <div className="text-center py-8">
              ...
              <button onClick={() => onNavigate("escanear")} className="text-blue-600 text-sm font-semibold hover:text-blue-700">
            ...
            <div className="space-y-3">
              {reservas.slice(0, 3).map((reserva) => (
                <div key={reserva.id} className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-gray-800">Sala {reserva.sala}</span>
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full flex items-center gap-1">
                      <HiCheckCircle className="text-sm" /> Activa
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                    <div className="flex items-center gap-1"><HiClock className="text-base" /> Bloque {reserva.bloque}</div>
                    <div className="flex items-center gap-1"><HiCalendar className="text-base" /> {reserva.fecha}</div>
                  </div>
                </div>
              ))}
              {reservas.length > 3 && (
                <p className="text-center text-sm text-gray-500 pt-2">+{reservas.length - 3} reserva(s) más</p>
              )}
            </div>
          )}
        </Card>

        {/* Botón Admin para Generador QR */}
